...
- **SQLite 方言**（`-d sqlite` / `-d sqlite3` / `-d 5`）：双引号标识符、`||` 拼接、`CHAR(10)` 换行、
  无 `N` 前缀、无反斜杠转义。
  **日期不加包装**：SQLite 没有日期类型，ISO-8601 文本就是它的规范表示，`date()` / `strftime()`
  能直接识别。规律是「只有真正需要包装的方言才加前缀」—— 目前仍只有 Oracle 需要 `TO_DATE`。
- **CTE 体内缩进**：`union` + `wrap = cte` 时，`WITH ... AS (` 里的 `SELECT` 与 `UNION ALL`
  缩进一级、闭合括号回到行首（对齐 sqlfluff 等格式化工具的默认风格）。
  `--wrap plain` **不缩进** —— 那是给「嵌进已有 SQL」用的，缩进交给调用方按所在层级对齐。
- **`infer_types` 支持 `auto`，并成为默认值**（配置文件三态：`auto` / `true` / `false`，
  命令行仍是 `--infer-types` / `--no-infer-types` 强制开关）。
- `reader.has_type_info()`：判断该扩展名的数据是否自带单元格类型信息。
- 方言清单收敛到单一来源 `dialects.ORDER`，菜单/编号/帮助文案全部从它派生，不再各写一份。

### 变更（行为变化，请留意）

- **CTE 产物的缩进变了**（见上）。产物文本与 0.3.x 不再逐字节相同，语义完全一致。
- **CSV 输入默认会还原数字列**。0.3.x 里 CSV 读出来全是文本，于是所有列都按字符串输出，
  `SUM()` 在 SQL Server / PostgreSQL 直接报错；0.4.0 起 `auto` 会把**整列都能无损解析成数字**的列
  还原成数字字面量。要回到旧行为：`--no-infer-types`，或配置 `infer_types = false`。
- **`auto` 只对 CSV 生效**，Excel 源不受影响：xlsx/xls 的单元格自带类型，写成文本就是用户有意的文本，
  工具不该改写它已经明确表达过的意图。要让 Excel 源也推断，用显式 `--infer-types`。
- **推断新增两条安全边界**（宁可留在文本，也不要造出会报错或丢信息的字面量）：
  1. **有效数字超过 15 位的不转**。15 是 Excel 的精度上限；越过这条线的"数字"几乎一定是编号，
     而且 19 位以上会超出三库的 `bigint` 范围 —— 硬转成数字字面量会让灌库直接报错。
  2. **转成 `float` 会丢尾数的不转**（用 `Decimal` 判断）。
  已知边界：**11 位手机号这类能无损装进 `bigint` 的编号仍会被转成数字**，需要时用 `--no-infer-types`。
- **`--all-string` 时不再做数字转换**。此前 `--infer-types --all-string` 会先转数字、再全部字符串化，
  顺带把数字列里的空格子改成 `NULL` —— 与「全部按字符串」自相矛盾。
...
- **渲染层抽出「输出写法」注册表**：`sqlgen.py` 的 `_render()` 不再于中段用
  ...
  加一种写法 = 加一个函数 + 注册一行。模块导入时校验 `RENDERERS` 与 `FORMATS` 不漂移。
...
- 新增 `tests/test_sqlite.py`（38 条）：**把产物真灌进标准库 `sqlite3` 再逐格核对** ——
  行数、列名、值、`typeof()` 存储类、多行文本的真换行、空白数字格变 `NULL`、空串不等于 `NULL`、
  前导零编号不被吃掉。因为 `sqlite3` 是标准库，这条端到端链路**会进 CI**，
  不像 SQL Server / Oracle 那样依赖外部容器。
- 新增 CTE 版式、`auto` 判定、数字推断安全边界、方言注册表一致性的用例；
  更新既有的 CTE 断言（原断言写死了不带缩进的形态）。单测 **186 → 257 条**。
- **E2E 差异审计**：本次有意的产物变化只有「CTE 缩进」与「CSV 数字推断」两类，
  所以与 0.3.2 基线**逐文件**比对，把每一处差异归因到这两类，并额外校验一条不变量 ——
  产物头注释里的「按字符串输出的列」清单只能变短、不能变长（推断只会让列移出该清单）。
  结果：49 个 SQL 产物（6 一致 / 36 仅缩进 / 6 推断 / 1 推断+空白转 NULL）与 60 个用例日志
  **零意外差异**。0.3.2 基线已归档到私有目录 `数据库测试/archive/baseline-0.3.2/`。
- **真库实测（VM 三库）**：把 0.4.0 产物灌进 SQL Server 2019 / PostgreSQL / MySQL 8，
  `union-cte` 与 `insert` × 三库共 **6 组全部 `rc=0`**，读回逐项一致：
  `SUM(数量)=11`、`SUM(金额)=3931.37`、空白数字格 `count(NULL)=1`、
  编号列 `0001,0002,0003` 保持字符类型（`nvarchar` / `text` / `varchar`）、
  数量列落成 `int` / `integer` / `bigint`、多行文本含真换行、空串仍是空串（不等于 `NULL`）。
  **`SUM()` 能算通就是 0.3.2 那个 P1 的正面证据** —— 该问题在 0.3.2 里只是「提供了开关」，
  这次才验证了「按数字输出」真的能被三库无损接受。
