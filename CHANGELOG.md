...
- **`-d` 传无法识别的方言名会静默回退成 SQL Server**。`-d myslq` / `-d db2` / `-d psql`
  都会若无其事地生成一份 **SQL Server 的 SQL** —— 产物语法完全正确、只是方言错了，
  往往要到灌库时才暴露，更糟的是被隐式转换掩盖过去，看起来像"跑通了"。
  现在 `dialects.resolve()` 抛 `UnknownDialect`：命令行与配置**报错退出（码 1）并列出可选值**，
  交互向导**重新询问**（打错字不该崩，更不该悄悄换成默认方言）。
  配置里的方言同样受校验，报错信息会指明来源是 `--dialect` 还是 `配置项 dialect`。

### 变更（行为变化）

- **`-d ''` 现在报错**。shell 里写 `-d "$DIALECT"` 而变量为空，以前被当成「没给 `-d`」
  而静默沿用配置值 —— 空变量恰恰是最容易静默用错方言的场景。
- 只校验**将会生效**的那个方言：命令行给了 `-d` 就以它为准，配置里的旧值不再追究
  （那是另一回事，不该拦住一次已经明确指定了方言的调用）。
- 校验发生在**读文件之前**，拼错时不必等一张大表读完才发现。

### 测试

- 单测 257 → 265：`resolve` 的未知值、报错文案需含全部可选值、别名提示须从 `ALIASES` 派生；
  以及 CLI 层的 `-d db2` / `-d ''` / 配置写坏方言 / 命令行覆盖配置四条。
